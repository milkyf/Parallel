		...
		*/
		...
		for(int j = 0; j < 3; j++)
		{
			...
			*/
			...
			if(maxVal > 0.9)
			{
				cv::rectangle(gray_img, roi_rect, Scalar(0,255,255), 3);// 探索結果の場所に矩形を描画
				resize(gray_img, gray_img, Size(gray_img.cols/4, gray_img.rows/4));//出力される画像が大きいので、出力画像を4分の1に縮小
				imshow(ship[i],gray_img);//第1引数の名前のウィンドウに，第2引数のMatクラスのインスタンスの中にある画像を表示する．
				resize(gray_img, gray_img, Size(gray_img.cols*4, gray_img.rows*4));//画像をもとの大きさに戻しておく(でないとfor文で繰り返されるうちにどんどん小さくなってしまう)
				waitKey(1);//カッコの中はミリ秒
			}
		}
	}
